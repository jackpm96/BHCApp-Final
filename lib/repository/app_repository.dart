...
class AppRepository {
  ...
  Future<SearchResponse> searchByKeyword(
      ...
      onError(error)) async {
    print("Search API Called and error is ==");
...
String a="myblackhistorycalendar.com";
    var url = Uri.https(a,
        "/wp-json/wcra/v1/search_by_keyword/", queryParameters);
  ...
  Future<FavResponse> getFavoritesPost(
      String token,
      String userId,
      onResponse(FavResponse list),
      onError(error)) async {
    ...
    String a="myblackhistorycalendar.com";
    var url = Uri.https(a,
        "/wp-json/wcra/v1/favourite_post_get/", queryParameters);
    print("url => " + url.toString());
    final response = await client.get(url,headers: {'Content-type': 'application/json'});
    if (response.statusCode == 200) {
      try {
