...
class _SignInScreenState extends State<SignInScreen> {
  ...
  Future<void> _signInWithApple() async {
    try {
      ...
      if (FirebaseAuth.instance.currentUser.displayName == "" || FirebaseAuth.instance.currentUser.displayName == null) {
        await FirebaseAuth.instance.currentUser.updateDisplayName("${appleCredential.givenName} ${appleCredential.familyName}");
      }
      ...
      if (user.credential != null) {
        ...
        var accessToken = {"access_token": token.token, "token_type": "bearer", "expires_in": "3600"};
        var body = {
        ...
        socialLoginApple(body).then((value) {
          ...
          if (value.code == 200) {
            ...
            Prefs.setName(FirebaseAuth.instance.currentUser.displayName.toString());
            ...
            if (FirebaseAuth.instance.currentUser.displayName.toString().split(" ").length > 0) {
              Prefs.setFirstName(FirebaseAuth.instance.currentUser.displayName.toString().split(" ").first);
              Prefs.setLastName(FirebaseAuth.instance.currentUser.displayName.toString().split(" ").last);
            } else {
              Prefs.setFirstName(FirebaseAuth.instance.currentUser.displayName.toString());
              Prefs.setLastName("");
            }
  ...
  Future<void> _signInWithGoogle() async {
    try {
      ...
      if (e.toString().contains("FACEBOOK_PROVIDER_ALREADY_PRESENT")) {
        CommonWidgets.buildSnackbar(context, "You are already registered with your Facebook account, please sign in using your Facebook!");
      } else {
